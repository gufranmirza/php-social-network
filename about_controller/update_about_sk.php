...
if(isset($_POST['username']) && isset($_POST['old_fields']) && isset($_POST['new_fields'])) {
    ...
    $query = "UPDATE `profile_skills` SET `skills_info`= '$new' WHERE `user_uid` = '$user' && `skills_info` = '$old' ";
...
if(isset($_POST['add_ed']) && !empty($_POST['add_ed']) && isset($_POST['user'])) {
    ...
    $user = $_POST['user'];
    $add_ed = $_POST['add_ed'];
    $query = "INSERT INTO `profile_skills`(`id`, `skills_info`, `user_uid`) VALUES ('','$add_ed','$user')";
    $query_run = mysqli_query($conn, $query);

    $query2 = "SELECT `id` , `skills_info` FROM `profile_skills` WHERE `skills_info` = '$add_ed' && `user_uid` = '$user'";
    $query_run2 = mysqli_query($conn, $query2);
    $rest = mysqli_fetch_assoc($query_run2);
    $id = $rest['id'];
    $fields = $rest['skills_info'];


    echo '
            
    <div class="p_apc_h_data">
        <i class="fa fa-plus-square fa-lg clr_back apc_h_data' . $id . ' "><p>' . $fields . ' </p>
